...
<<<<<<< Updated upstream:src/org/kinkydesign/decibell/db/derby/DerbyTablesGenerator.java
...
=======
        Annotation tableName = c.getAnnotation(TableName.class);
        if (tableName == null) {
            table.setTableName(connector.getUser() + DOT + c.getName().replace(DOT, UNDERSCORE));
        } else {
            table.setTableName(((TableName) tableName).value());
        }
>>>>>>> Stashed changes:src/org/kinkydesign/decibell/db/derby/DerbyTablesGenerator.java
...
=======
                table.setTableName(connector.getUser() + DOT
                        + (registry.get(f.getDeclaringClass()).getTableName()
                        + UNDERSCORE + LogicalOperator.AND + UNDERSCORE + registry.get(carg).getTableName() + UNDERSCORE + ON + UNDERSCORE + f.getName()).replaceAll(DOT_REG, UNDERSCORE));
